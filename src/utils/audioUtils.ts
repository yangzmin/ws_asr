 ...
 */
export function audioBufferToWav(audioBuffer: AudioBuffer, sampleRate: number = 16000): ArrayBuffer {
 ...
 * @returns 重采样后的AudioBuffer
 */
export function resampleAudioBuffer(audioBuffer: AudioBuffer, targetSampleRate: number): AudioBuffer {
 ...
 * @param sampleRate - 采样率
 * @param channels - 声道数
 * @param includeWavHeader - 是否包含WAV头，默认true
 * @returns Base64编码的WAV数据或PCM数据
 */
export function audioDataToWavBase64(audioData: Float32Array, sampleRate: number = 16000, channels: number = 1, includeWavHeader: boolean = true): string {
