...
// IMGatewayService 用于 gRPC 注册时的类型校验（空接口即可）
type IMGatewayService interface{}

// IMGatewayServer gRPC 总线 + WS 服务
type IMGatewayServer struct {
	...
	gatewayStream grpc.ServerStream
}

func NewIMGatewayServer(cfg *configs.Config, logger *utils.Logger) *IMGatewayServer {
...
func (s *IMGatewayServer) registerService() {
	svc := &grpc.ServiceDesc{
		ServiceName: "im.IMGateway",
		HandlerType: (*IMGatewayService)(nil),
		Streams: []grpc.StreamDesc{
			{
				StreamName:    "MessageGateway",
				ServerStreams: true,
				ClientStreams: true,
				Handler:       s.messageGatewayHandler,
			},
		},
	}
...
func (s *IMGatewayServer) HandleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		defer func() {
			...
			s.gatewaySend(&ImMessage{Event: "session_close", SessionID: sessionID})
			_ = conn.Close()
		}()
		for {
...
// waitGatewayConnected 在指定超时时间内等待 AI 总线连接建立
